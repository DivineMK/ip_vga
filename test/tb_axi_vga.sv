...
module tb_axi_vga;
  ...
  // AXI interface
  `AXI_TYPEDEF_ALL(axi_vga_tb, logic [AXIAddrWidth-1:0], logic [AXIIdWidth-1:0],
  ...
  initial begin : frame_capture
    ...
    forever begin
      ...
      if (vsync_prev == cfg.control.vsync_pol && i_axi_vga.vsync_o == ~cfg.control.vsync_pol) begin
        vsync_porch = 0;
        hsync_porch = 0;
        row = 0;
        col = 0;
  ...
  task read_bmp(input string file);
    ...
    tmp = $fgetc(fd);
    tmp = $fgetc(fd);
    tmp = $fgetc(fd);  // reserved
    data_offset = 0;  // data offset
    data_offset |= $fgetc(fd);
    data_offset |= $fgetc(fd) << 8;
    data_offset |= $fgetc(fd) << 16;
    data_offset |= $fgetc(fd) << 24;

    // DIB header
    tmp   = $fgetc(fd);
    tmp   = $fgetc(fd);
    tmp   = $fgetc(fd);
    tmp   = $fgetc(fd);  // header size
    ...
    if (height != FrameHeight || width != FrameWidth) begin
      $error("Bitmap has wrong dimensions (is %0dx%0d, should %0dx%0dx)", width, height,
    ...
    tmp = $fgetc(fd);  // planes
    tmp = $fgetc(fd);
    tmp = $fgetc(fd);  // bpp (assumes 24bit)
